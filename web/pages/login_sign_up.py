...
class MainHandler(webapp2.RequestHandler):
    def get(self):
        user = None # בודקים אם העוגיה טובה ותקינה
        if self.request.cookies.get('our_token'):    #the cookie that should contain the access token!
            user = User.checkToken(self.request.cookies.get('our_token')) ##מחזיר אם יש יוזר כזה או שהטוק לא תקין

        template_variables = {}
        if user: #אם יש לי יוזר- בדיקה
            template_variables['user'] = user.email

        html = template.render('templates/index.html', template_variables)
        self.response.write(html)
#אנו צריכים לבדוק משתמש וסיסמא. אנו נלך למסד נתונים ונשאל אם זה מתאים. ניתן את הטוקן הטוב אחרת נאמר שהפרטים לא נכונים
#
